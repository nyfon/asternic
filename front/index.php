...
$desc_curhour1 = date('Y-m-d H:00', strtotime("-1 hour")); // description last hour
$desc_curhour2 = date('H:59', strtotime("-1 hour")); // description last hour
...
<script type="text/javascript">
	...
	function setdates(start,end) {
		...
		var start_hour  = start.substr(11,2);

		var end_year  = end.substr(0,4);
		var end_month = end.substr(5,2);
		var end_day   = end.substr(8,2);
		var end_hour  = end.substr(11,2);

		hstart = MWJ_findSelect( "hour1" ), dstart = MWJ_findSelect( "day1" ), mstart = MWJ_findSelect( "month1" ), ystart = MWJ_findSelect( "year1" );
		hend   = MWJ_findSelect( "hour2" ), dend   = MWJ_findSelect( "day2" ), mend   = MWJ_findSelect( "month2" ), yend   = MWJ_findSelect( "year2" );
		...
		sh = start_hour - 1; sh = sh + 1; // type change
		eh = end_hour - 1; eh = eh + 1;   // type change

	    hstart.options[sh].selected = true;
	    hend.options[eh].selected = true;
...
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
//
echo "<a href=\"javascript:setdate2('$start_dayb1', '$end_dayb1')\" ";
echo "    alt=\"$start_dayb1 - $end_dayb1\" ";
echo "  title=\"$start_dayb1 - $end_dayb1\" ";
echo " > Вчера  </a> | ";
//
echo "<a href=\"javascript:setdate2('$start_curhour', '$end_curhour')\"";
echo "    alt=\"$desc_curhour1 - $desc_curhour2\" ";
echo "  title=\"$desc_curhour1 - $desc_curhour2\"";
echo "       >($desc_curhour1 - $desc_curhour2)</a> | ";
//
echo "<BR>";
?>
<BR>
...
<TD>
		...
		<?php
for ($a = 0; $a < 24; $a++) {
	echo "<option value=" . str_pad($a, 2, '0', STR_PAD_LEFT) . "";
	if ($fstart_hour == $a) {echo " selected ";}
	echo ">" . str_pad($a, 2, '0', STR_PAD_LEFT) . "</option>\n";
}
?>
        </select>
